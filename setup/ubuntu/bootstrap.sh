...
REDASH_BASE_PATH=$HOME/.redash
REDASH_BRANCH="${REDASH_BRANCH:-master}" # Default branch/version to master if not specified in REDASH_BRANCH env var
REDASH_VERSION=${REDASH_VERSION-5.0.0-beta} # Install latest version if not specified in REDASH_VERSION env var
...
extract_redash_sources() {
    ...
    ln -nfs $REDASH_BASE_PATH/.env $REDASH_BASE_PATH/current/redash-$REDASH_VERSION/.env
}
...
install_python_packages() {
    ...
    pip install -r $REDASH_BASE_PATH/current/redash-$REDASH_VERSION/requirements.txt
    pip install -r $REDASH_BASE_PATH/current/redash-$REDASH_VERSION/requirements_all_ds.txt
}
...
create_database() {
    ...
    cd $REDASH_BASE_PATH/current/redash-$REDASH_VERSION/
